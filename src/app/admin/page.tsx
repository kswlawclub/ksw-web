  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f6f2ea] text-[#061426]">
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-4 px-4 py-10 sm:px-6 md:grid-cols-2 lg:px-10">
        {modules.map((module) => (
          <article
            className="rounded-lg border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/10"
            key={module}
          >
            <div className="mb-4 h-0.5 w-12 rounded-full bg-[#d8ad45]" />
            <h2 className="text-2xl font-black text-[#061426]">{module}</h2>
            <p className="mt-3 text-sm font-semibold text-slate-600">Coming Soon</p>
          </article>
        ))}
      </section>
    </main>
