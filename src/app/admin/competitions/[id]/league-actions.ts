...
import { effectiveMatchweekOrNull, reconcileEffectiveMatchweekStatus } from "@/lib/league-matchweek-rescheduling";
...
async function reconcileMatchweekCompletion(supabase: NonNullable<ReturnType<typeof getSupabaseAdmin>>, competitionId: string, fixtureVersion: number, matchweek: number) {
  const matches = await supabase
    .from("matches")
    .select("id, matchweek, scheduled_matchweek, status")
    .eq("league_id", competitionId)
    .eq("league_fixture_version", fixtureVersion);
  if (matches.error) {
    console.error("standard league matchweek completion lookup failed", matches.error);
    return { error: "ไม่สามารถตรวจสอบสถานะ Matchweek หลังบันทึกผลได้", state: null };
  }
  const effectiveMatches = (matches.data ?? []).map((match) => ({
    id: match.id,
    originalMatchweek: match.matchweek,
    scheduledMatchweek: match.scheduled_matchweek,
    status: match.status,
  }));
  const states = await loadMatchweekState(supabase, competitionId, fixtureVersion);
  if (states.error) return { error: states.error, state: null };
  const currentState = states.states.find((state) => state.matchweek === matchweek);
  if (!currentState) return { error: "ไม่พบสถานะ Matchweek ที่ต้องอัปเดต", state: null };
  const status = reconcileEffectiveMatchweekStatus(currentState.status, effectiveMatches, matchweek);
  if (status === currentState.status) return { error: "", state: currentState };
  const reconciled = await supabase
    .from("competition_league_matchweeks")
    .upsert({ competition_id: competitionId, fixture_version: fixtureVersion, matchweek, status, updated_at: new Date().toISOString() }, { onConflict: "competition_id,fixture_version,matchweek" })
    .select(matchweekColumns)
    .single();
  if (reconciled.error) {
    console.error("standard league matchweek completion persistence failed", reconciled.error);
    return { error: "ไม่สามารถอัปเดตสถานะ Matchweek หลังบันทึกผลได้", state: null };
  }
  return { error: "", state: matchweekFromRow(reconciled.data as Row) };
}
...
export async function saveStandardLeagueMatch(competitionId: string, payload: {
  ...
  const effectiveMatchweek = effectiveMatchweekOrNull(match.data.matchweek, match.data.scheduled_matchweek);
  if (effectiveMatchweek === null || !Number.isInteger(effectiveMatchweek)) return { error: "คู่นี้ไม่มี Matchweek ที่ยืนยันได้", ok: false };
  const refreshedMatchweeks = await loadMatchweekState(verified.supabase, competitionId, config.fixtureVersion);
  if (refreshedMatchweeks.error) return { error: refreshedMatchweeks.error, ok: false };
  const matchweekState = refreshedMatchweeks.states.find((state) => state.matchweek === effectiveMatchweek);
  if (payload.status === "finished" && matchweekState?.status !== "confirmed" && matchweekState?.status !== "completed") return { error: `ต้องยืนยันคู่แข่งขัน Matchweek ${effectiveMatchweek} ก่อนจึงจะบันทึกผลจบการแข่งขันได้`, ok: false };
