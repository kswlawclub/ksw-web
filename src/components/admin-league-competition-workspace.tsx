...
import { canRescheduleMatch, isRescheduledMatch, isSupplementalMatchweek, movedMatchweekCounts, sortRescheduleHistory, validateRescheduleDraft } from "@/lib/league-matchweek-rescheduling";
  ...
  return (
    <div>
      <label className="text-xs font-bold">สนาม
        <select className="mt-1 w-full rounded border p-2" onChange={(event) => {
      ...
      {mode === "Other" ? <label className="mt-2 block text-xs font-bold">สนามอื่น<input className="mt-1 w-full rounded border p-2" onChange={(event) => onChange(event.target.value)} value={parts.other} /></label> : null}
    </div>
  );
}
...
function MatchweekSection({
  ...
  const confirm = async () => {
    ...
    setSaving(false);
    if (!result?.ok || !result.matchweekState) { setError(result?.error ?? "Matchweek นี้ยังไม่พร้อมยืนยันคู่แข่งขัน"); return; }
  ...
  return <article className="rounded-lg border border-slate-200 bg-white p-4">
    <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-black">{supplemental ? "นัดตกค้าง / สัปดาห์เพิ่มเติม" : "Matchweek"} {state.matchweek}</h3><span className={`rounded-full px-2 py-1 text-xs font-bold ${statusClass(state.status)}`}>{statusLabel(state.status)}</span></div>{state.status === "completed" ? <button className="text-xs font-bold" onClick={() => setCollapsed((current) => !current)} type="button">{collapsed ? "แสดงผล" : "พับผล"}</button> : null}</div>
    ...
    {state.status !== "completed" && initialMatches.length ? <p className="mt-2 text-xs font-semibold text-slate-600">พร้อม {readiness.readyMatches}/{readiness.totalMatches} คู่ · รอกำหนด {readiness.incompleteMatches} คู่{state.status !== "confirmed" ? " · สามารถยืนยันคู่แข่งขันได้ และเติมวัน เวลา หรือสนามภายหลัง" : ""}</p> : null}
    ...
    {state.status !== "completed" || !collapsed ? <>
      ...
      const result = scores[match.id] ?? { away: match.away_score?.toString() ?? "", home: match.home_score?.toString() ?? "", status: match.status === "finished" ? "finished" : "scheduled" };
      const finished = ["finished", "completed"].includes(match.status);
      const locked = state.status === "completed" || (finished && editingFinishedMatchId !== match.id);
