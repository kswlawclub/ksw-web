...
export function AdminLeagueCompetitionWorkspace({ competitionId, competitionName, competitionStatus, initialConfig, initialMatches, initialMatchweeks, rescheduleHistory: initialRescheduleHistory, teams }: { competitionId: string; competitionName: string; competitionStatus: string; initialConfig: StandardLeagueConfig | null; initialMatches: AdminCompetitionMatch[]; initialMatchweeks: StandardLeagueMatchweek[]; rescheduleHistory: StandardLeagueRescheduleHistory[]; teams: AdminCompetitionMatchTeam[] }) {
  ...
  const expectedFixtureCount = config ? teams.length * (teams.length - 1) / 2 * config.legs : 0;
  const championResolution = config ? resolveStandardLeagueChampion({ config, expectedFixtureCount, matches: leagueMatches.map((match) => ({ awayScore: match.away_score, fixtureKey: match.league_fixture_key ?? null, homeScore: match.home_score, status: match.status })), standings: standings.rows }) : { reason: "ยังไม่ได้ตั้งค่าลีก", status: "pending" as const };
  const effectiveWeeks = Array.from(new Set(leagueMatches.map(effectiveWeek).filter((week) => week > 0)));
  const incompleteWeeks = effectiveWeeks.filter((week) => matchweeksByNumber.get(week)?.status !== "completed");
  const unresolvedFinalRequirements = [
    leagueMatches.length !== expectedFixtureCount ? `จำนวนคู่แข่งขันไม่ครบ (${leagueMatches.length}/${expectedFixtureCount})` : "",
    leagueMatches.filter((match) => !["finished", "completed"].includes(match.status)).length ? "ยังมีผลการแข่งขันที่ไม่จบ" : "",
    incompleteWeeks.length ? `Matchweek ${incompleteWeeks.join(", ")} ยังไม่แข่งขันครบ` : "",
    standings.rows.length !== teams.length || new Set(standings.rows.map((row) => row.teamId)).size !== teams.length ? "ตารางคะแนนมีข้อมูลทีมไม่ครบหรือซ้ำ" : "",
  ].filter(Boolean);
  const eligibleChampionIds = championResolution.status === "champion"
    ? [championResolution.championTeamId]
    : championResolution.status === "needs_admin_resolution"
      ? championResolution.candidateTeamIds
      : [];
  const championConfirmed = Boolean(config?.championTeamId && config.championConfirmedByLabel && eligibleChampionIds.includes(config.championTeamId));
  const totalGoals = leagueMatches.reduce((sum, match) => sum + (match.home_score ?? 0) + (match.away_score ?? 0), 0);
  const finishedFixtureCount = leagueMatches.filter((match) => ["finished", "completed"].includes(match.status)).length;
  ...
  const confirmChampion = async () => {
    const championTeamId = selectedChampionId || (championResolution.status === "champion" ? championResolution.championTeamId : "");
    const result = await runAction({ errorMessage: (value) => !value.ok ? value.error ?? "ยืนยันแชมป์ไม่สำเร็จ" : null, id: `league-champion-confirm:${competitionId}`, loadingMessage: "กำลังยืนยันแชมป์…", successMessage: "ยืนยันแชมป์แล้ว" }, () => confirmStandardLeagueChampion(competitionId, championTeamId, championReason));
    if (!result?.ok) { setError(result?.error ?? "ยืนยันแชมป์ไม่สำเร็จ"); return; }
    setConfig(result.config ?? null); router.refresh();
  };
  const completeSeason = async () => {
    if (!window.confirm("ตรวจสอบแล้วว่าผลการแข่งขันและแชมป์ถูกต้อง ต้องการปิดฤดูกาลใช่หรือไม่")) return;
    const result = await runAction({ errorMessage: (value) => !value.ok ? value.error ?? "ปิดฤดูกาลไม่สำเร็จ" : null, id: `league-complete:${competitionId}`, loadingMessage: "กำลังปิดฤดูกาล…", successMessage: "ปิดฤดูกาลแล้ว" }, () => completeStandardLeagueCompetition(competitionId));
    if (!result?.ok) { setError(result?.error ?? "ปิดฤดูกาลไม่สำเร็จ"); return; }
    router.refresh();
  };
  ...
  return <section className="mx-auto w-full max-w-7xl space-y-5 px-4 pb-12 sm:px-6 lg:px-10">
    ...
    <section className="rounded-lg border bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-xl font-black">Champion</h2><span className={`rounded-full px-2 py-1 text-xs font-black ${competitionStatus === "completed" ? "bg-emerald-100 text-emerald-800" : championConfirmed ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-900"}`}>{competitionStatus === "completed" ? "Completed · การแข่งขันเสร็จสิ้น" : championConfirmed ? "ยืนยันแล้ว" : "รอยืนยัน"}</span></div>{unresolvedFinalRequirements.length ? <div className="mt-3 rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950"><p className="font-black">ยังไม่พร้อมสรุปแชมป์</p><ul className="mt-1 list-disc pl-5">{unresolvedFinalRequirements.map((item) => <li key={item}>{item}</li>)}</ul></div> : <><div className="mt-3 grid gap-2 sm:grid-cols-3">{standings.rows.slice(0, 3).map((row, index) => <div className="rounded border bg-slate-50 p-3" key={row.teamId}><p className="text-xs font-bold text-slate-500">{index === 0 ? "Champion" : index === 1 ? "Runner-up" : "Third place"}</p><p className="mt-1 font-black">{row.teamName}</p><p className="mt-1 text-xs text-slate-600">{row.points} คะแนน · {row.wins} ชนะ {row.draws} เสมอ {row.losses} แพ้ · ได้ {row.goalsFor} เสีย {row.goalsAgainst} · {row.goalDifference >= 0 ? "+" : ""}{row.goalDifference}</p></div>)}</div>{championResolution.status === "needs_admin_resolution" ? <div className="mt-3 rounded border border-amber-300 bg-amber-50 p-3"><p className="text-sm font-black text-amber-950">ต้องให้แอดมินตัดสิน: {championResolution.reason}</p><label className="mt-2 block text-sm font-bold">เลือก Champion<select className="mt-1 w-full rounded border p-2" disabled={competitionStatus === "completed"} onChange={(event) => setSelectedChampionId(event.target.value)} value={selectedChampionId}>{championResolution.candidateTeamIds.map((teamId) => <option key={teamId} value={teamId}>{nameFor(teamMap, teamId)}</option>)}</select></label><label className="mt-2 block text-sm font-bold">เหตุผลการตัดสิน<input className="mt-1 w-full rounded border p-2" disabled={competitionStatus === "completed"} onChange={(event) => setChampionReason(event.target.value)} value={championReason} /></label></div> : null}{championResolution.status === "champion" && !championConfirmed ? <p className="mt-3 text-sm text-slate-700">ระบบคำนวณผู้นำตาม คะแนน · ผลต่างประตู · ประตูได้ · จำนวนชนะ: <strong>{nameFor(teamMap, championResolution.championTeamId)}</strong></p> : null}{championConfirmed ? <p className="mt-3 text-sm font-bold text-emerald-800">ยืนยันแชมป์: {nameFor(teamMap, config?.championTeamId ?? "")}{config?.championConfirmedByLabel ? ` · ${config.championConfirmedByLabel}` : ""}</p> : null}{competitionStatus !== "completed" && !championConfirmed ? <ActionButton actionId={`league-champion-confirm:${competitionId}`} className="mt-3 rounded bg-[#061426] px-3 py-2 text-sm font-black text-[#f4d58a]" disabled={!eligibleChampionIds.length} loadingLabel="กำลังยืนยันแชมป์…" onClick={confirmChampion} successLabel="ยืนยันแชมป์แล้ว" type="button">ยืนยันแชมป์</ActionButton> : null}{competitionStatus !== "completed" && championConfirmed ? <ActionButton actionId={`league-complete:${competitionId}`} className="mt-3 rounded bg-emerald-700 px-3 py-2 text-sm font-black text-white" loadingLabel="กำลังปิดฤดูกาล…" onClick={completeSeason} successLabel="ปิดฤดูกาลแล้ว" type="button">ปิดฤดูกาล</ActionButton> : null}</>}</section>
    <section className="rounded-lg border bg-white p-4"><h2 className="text-xl font-black">สรุปฤดูกาล</h2><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4"><p><span className="text-slate-500">จำนวนทีม</span><br /><strong>{teams.length} ทีม</strong></p><p><span className="text-slate-500">จำนวน Matchweek</span><br /><strong>{effectiveWeeks.length} สัปดาห์</strong></p><p><span className="text-slate-500">แมตช์ที่จบ</span><br /><strong>{finishedFixtureCount}/{leagueMatches.length} คู่</strong></p><p><span className="text-slate-500">ประตูรวม</span><br /><strong>{totalGoals} ประตู · {finishedFixtureCount ? (totalGoals / finishedFixtureCount).toFixed(2) : "0.00"} ต่อนัด</strong></p><p><span className="text-slate-500">Champion</span><br /><strong>{championConfirmed ? nameFor(teamMap, config?.championTeamId ?? "") : "รอยืนยัน"}</strong></p><p><span className="text-slate-500">Runner-up</span><br /><strong>{standings.rows[1]?.teamName ?? "-"}</strong></p><p><span className="text-slate-500">Third place</span><br /><strong>{standings.rows[2]?.teamName ?? "-"}</strong></p><p><span className="text-slate-500">ชนะมากที่สุด</span><br /><strong>{standings.rows[0] ? `${standings.rows[0].teamName} · ${standings.rows[0].wins} นัด` : "-"}</strong></p><p><span className="text-slate-500">ยิงประตูมากที่สุด</span><br /><strong>{[...standings.rows].sort((left, right) => right.goalsFor - left.goalsFor)[0] ? `${[...standings.rows].sort((left, right) => right.goalsFor - left.goalsFor)[0].teamName} · ${[...standings.rows].sort((left, right) => right.goalsFor - left.goalsFor)[0].goalsFor} ประตู` : "-"}</strong></p></div></section>
  </section>;
}
